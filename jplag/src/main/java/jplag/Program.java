...
public class Program implements ProgramI {
  ...
  private Hashtable<String, AllBasecodeMatches> htBasecodeMatches = new Hashtable<String, AllBasecodeMatches>(
      30);
  ...
  /**
   * Like the validSubmissions(), but this time all the submissions are returned as a string,
  ...
  public float compare_two(Submission s1, Submission s2) {
  ...
  private void createSubmissions() throws jplag.ExitException {
    ...
    try {
      ...
      throw new jplag.ExitException(
          "Unable to retrieve directory: " + options.root_dir + " Cause : " + e.toString());
    }
    Arrays.sort(list);
    ...
    for (int i = 0; i < list.length; i++) {
      ...
      if (file_dir.isDirectory()) {
        if (options.basecode.equals(subm_dir.getName())) {
          basecodeSubmission = new Submission(subm_dir.getName(), file_dir,
              options.read_subdirs,
              this, get_language());
        } else {
          submissions.addElement(
              new Submission(subm_dir.getName(), file_dir, options.read_subdirs, this,
                  get_language())); // -s
        }
      } else {
  ...
  private void externalCompare() throws jplag.ExitException {
    ...
    // Progress progress;
    // Result vector
    SortedVector<AllMatches> avgmatches = new SortedVector<AllMatches>(
        new AllMatches.AvgComparator());
    SortedVector<AllMatches> maxmatches = new SortedVector<AllMatches>(
  ...
  private void registerMatch(AllMatches match, int[] dist, SortedVector<AllMatches> avgmatches,
    ...
    if (!options.store_percent) {
      if ((avgmatches.size() < options.store_matches || avgpercent > avgmatches.lastElement()
      ...
      if (maxmatches != null && (maxmatches.size() < options.store_matches
      ...
      if (minmatches != null && (minmatches.size() < options.store_matches
  ...
  public void run() throws jplag.ExitException {
    ...
    if (options.exp) { // EXPERIMENT
      ...
      if (options.compare > 0) {
        ...
        switch (options.comparisonMode) {
          ...
          default:
            throw new ExitException(
                "Illegal comparison mode: \"" + options.comparisonMode + "\"");
        }
      }
    }
    closeWriter();
    ...
    str +=
        " basecode_dir = " + sp + toUTF8((options.basecode != null) ? options.basecode : "") + sp;
    str += " read_subdirs = " + sp + this.options.read_subdirs + sp;
    str += " clustertype = " + sp + this.options.getClusterTyp() + sp;
    str +=
        " store_matches = " + sp + this.options.store_matches + ((this.options.store_percent) ? "%"
            : "") + sp;
    String suf = "";
    ...
    try {
      FileWriter fw = new FileWriter(
          new File(this.options.result_dir + File.separator + "result.xml"));
      fw.write(str);
      fw.close();
    } catch (IOException ex) {
  ...
  private void specialCompare() throws jplag.ExitException {
    ...
    for (i = 0; i < (size - 1); i++) {
      // Result vector
      SortedVector<AllMatches> matches = new SortedVector<AllMatches>(
          new AllMatches.AvgComparator());
      ...
      for (Iterator<AllMatches> iter = matches.iterator(); iter.hasNext(); ) {
        ...
        if (once) {
          f.println(
              "<TR><TD BGCOLOR=" + this.report.color(match.percent(), 128, 192, 128, 192, 255, 255)
                  + ">" + s1.name
                  + "<TD WIDTH=\"10\">-&gt;");
          once = false;
        }
        ...
        f.println(" <TD BGCOLOR=" + this.report.color(match.percent(), 128, 192, 128, 192, 255, 255)
            + " ALIGN=center><A HREF=\"match" + matchIndex + ".html\">" + match.subName(other)
            + "</A><BR><FONT COLOR=\""
            + this.report.color(match.percent(), 0, 255, 0, 0, 0, 0) + "\"><B>(" + match
            .roundedPercent() + "%)</B></FONT>");
   ...
   */
  private void writeResults(int[] dist, SortedVector<AllMatches> avgmatches,
      SortedVector<AllMatches> maxmatches,
